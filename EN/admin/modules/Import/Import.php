...
class Import extends CodonModule {
    ...
    public function processexport() {
        ...
        foreach ($all_schedules as $s) {
            $line = "{$s->code},{$s->flightnum},{$s->depicao},{$s->arricao},{$s->alticao}," . "{$s->route},{$s->registration},{$s->flightlevel},{$s->distance}," .
                "{$s->deptime}, {$s->arrtime}, {$s->flighttime}, {$s->fnalt}, {$s->fnalt}, {$s->notes}, " . "{$s->price}, {$s->flighttype}, {$s->daysofweek}, {$s->enabled}, {$s->week1}, {$s->week2}, {$s->week3}, {$s->week4}";
    ...
    public function processimport() {
        ...
        while ($fields = fgetcsv($fp, 1000, ',')) {
            ...
            $route = $fields[5];
            $aircraft = $fields[6];
            $flightlevel = $fields[7];
            $distance = $fields[8];
            $deptime = $fields[9];
            $arrtime = $fields[10];
            $flighttime = $fields[11];
			$fnalt = $fields[12];
            $notes = $fields[13];
            $price = $fields[14];
            $flighttype = $fields[15];
            $daysofweek = $fields[16];
            $enabled = $fields[17];
            $week1 = $fields[18];
            $week2 = $fields[19];
            $week3 = $fields[20];
            ...
            $data = array(
                ...
                'arricao' => $arricao, 'alticao' => $alticao, 'route' => $route, 'aircraft' => $ac, 'flightlevel' => $flightlevel,
                'distance' => $distance, 'deptime' => $deptime, 'arrtime' => $arrtime,
                'flighttime' => $flighttime, 'fnalt' => $fnalt, 'daysofweek' => $daysofweek, 'notes' => $notes,
